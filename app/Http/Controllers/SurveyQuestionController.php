...
class SurveyQuestionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $filterCriteria = $request->get('criteria', 'all');
        ...
        $dbSections = SurveySection::with(['allQuestions' => function($query) use ($filterStatus, $filterCriteria, $filterType) {
            ...
            // Filter criteria_type via relasi criteria
            if ($filterCriteria === 'benefit') {
                $query->whereHas('criteria', fn($q) => $q->where('criteria_type', 'benefit'));
            } elseif ($filterCriteria === 'cost') {
                $query->whereHas('criteria', fn($q) => $q->where('criteria_type', 'cost'));
            }
        ...
        // Default questions tidak punya criteria, kosongkan jika filter benefit/cost
        if ($filterCriteria === 'benefit' || $filterCriteria === 'cost') {
            $defaultQuestionsCollection = collect([]);
        }
        ...
        if ($hasActiveFilter) {
            $sections = $sections->filter(function($section) {
                return is_array($section->allQuestions)
                    ? count($section->allQuestions) > 0
                    : $section->allQuestions->count() > 0;
            })->values();
        }
    ...
    public function createQuestion($sectionId)
    {
        ...
        $criterias = Criteria::orderBy('criteria_name')->get();

        return view('admin.questions.create-question', compact('section', 'criterias'));
    }
    ...
    public function storeQuestion(Request $request, $sectionId)
    {
        ...
        // Jika SAW aktif dan tipe linear_scale, criteria_id wajib diisi
        if ($request->boolean('enable_saw') && $request->question_type === 'linear_scale') {
            $validationRules['criteria_id'] = 'required|exists:criterias,id';
        }
    ...
    public function editQuestion($questionId)
    {
        ...
        $question  = SurveyQuestion::with(['section', 'criteria'])->findOrFail($questionId);
        $criterias = Criteria::orderBy('criteria_name')->get();

        return view('admin.questions.edit-question', compact('question', 'criterias'));
    }
    ...
    public function showLockForm()
    {
        ...
        return view('admin.questions.lock-confirm', compact(
            'totalSections',
            'totalQuestions',
    ...
    // Cek apakah sistem terkunci (private helper)
    private function isSystemLocked()
    {
        return SurveyPeriod::isLocked();
    }

    // Hentikan periode aktif
